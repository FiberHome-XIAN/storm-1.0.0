...
(defn add-shutdown-hook-with-force-kill-in-n-sec
  "adds the user supplied function as a shutdown hook for cleanup."
  [func n]
  (.addShutdownHook (Runtime/getRuntime) (Thread. #(func)))
  (log-message "----------add-shutdown-hook-with-force-kill-in-" n "-sec----------")
  (.addShutdownHook (Runtime/getRuntime) (Thread. #((sleep-secs n)
                                                    (.halt (Runtime/getRuntime) 20)))))
