...
(ns org.apache.storm.daemon.supervisor
  ...
  (:import [org.apache.storm.scheduler ISupervisor]
           ...
           [org.apache.storm.daemon Shutdownable LaunchAttemptCounter]
  ...
  (:import [org.apache.storm.generated AuthorizationException KeyNotFoundException WorkerResources LocalAssignment])
    ...
    (remove nil?
      (dofor [[port assignment] reassign-executors]
        (let [id (new-worker-ids port)
              ...
              executors (:executors assignment)]
          (if-not (< (LaunchAttemptCounter/getAttemptCount storm-id executors) 5)
            (log-message "Launch assignment [" assignment "] attempt count: " (LaunchAttemptCounter/getAttemptCount storm-id executors) " already reach MAX_ATTEMPT_COUNT!")
            ...
            (do
              (log-message "Launching worker with assignment "
                (get-worker-assignment-helper-msg assignment supervisor port id)
                           " with attempt count: " (LaunchAttemptCounter/getAttemptCountAndInc storm-id executors))
...
(defn sync-processes [supervisor]
  (let [conf (:conf supervisor)
    ...
    ;; clear assignment lauch attempt
    (LaunchAttemptCounter/clearAttempts assigned-executors)
